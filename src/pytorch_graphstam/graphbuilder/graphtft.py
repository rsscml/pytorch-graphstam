...
from ..utils.setcover_functions import *
from ..models.stgnn_tft import STGNN
...
class graphmodel:
    def __init__(self,
        ...
        if tweedie_p_range is None:
            tweedie_p_range = [1.01, 1.95]
        if rolling_features_list is None:
    ...
    def create_new_keys(self, df):
        ...
        logger.info("created new key cols: ", self.new_key_cols)
        logger.info("created new key to subkeys mapping: ", self.key_levels_dict)
        logger.info("covariates applied at this key level: ", self.covar_key_level)
    ...
    def stack_key_level_dataframes(self, df, df_keybom):
        df_stack_list = []
        for (k, v), (k2, v2), (k3, v3) in zip(self.key_levels_dict.items(), self.key_targets_dict.items(),
                                              self.key_weights_dict.items()):
            if (k == k2) and (k == k3):
        ...
        logger.info("Derived key_level weights: \n", key_levels_weight_dict)
    ...
    def power_optimization_loop(self, df):
        ...
        try:
            ...
            logger.info("initializing with power: ", power)
    ...
    def pad_dataframe(self, df, dateindex):
        ...
        df = df.groupby(self.id_col, sort=False).apply(lambda x: fillgrpid(x).fillna(self.pad_constant)).reset_index(
            drop=True)
    ...
    def parallel_pad_dataframe(self, df):
        ...
        groups = df.groupby([self.id_col])
        padded_gdfs = Parallel(n_jobs=self.PARALLEL_DATA_JOBS, batch_size=self.PARALLEL_DATA_JOBS_BATCHSIZE,
                               backend=backend, timeout=timeout)(
            delayed(self.pad_dataframe)(gdf, dateindex) for _, gdf in groups)
    ...
    def preprocess(self, data):
        ...
        if null_status:
            ...
            logger.info("NaN column(s): ", null_cols)
        ...
        logger.info("preprocessed known_onehot_cols: ", self.known_onehot_cols)
        logger.info("preprocessed unknown_onehot_cols: ", self.unknown_onehot_cols)
        logger.info("preprocessed global_context_onehot_cols: ", self.global_context_onehot_cols)
        logger.info("preprocessed temporal_known_num_col_list: ", self.temporal_known_num_col_list)
        logger.info("preprocessed temporal_unknown_num_col_list: ", self.temporal_unknown_num_col_list)
        logger.info("Total Keys across hierarchy: ", df[self.id_col].nunique())
    ...
    def create_snapshot_graph(self, df_snap, period):
        ...
        df_snap_target = df_snap.groupby([self.id_col, self.time_index_col]).agg({self.target_col: np.sum}).unstack(
            level=self.time_index_col).reset_index(drop=True)

        # target mask processing
        df_snap_mask = df_snap.groupby([self.id_col, self.time_index_col]).agg({'y_mask': np.sum}).unstack(
            level=self.time_index_col).reset_index(drop=True)

        # outlier mask processing
        df_snap_outlier_mask = df_snap.groupby([self.id_col, self.time_index_col]).agg(
            {'y_outlier_mask': np.sum}).unstack(level=self.time_index_col).reset_index(drop=True)

        # recency_weights processing
        df_snap_recency_wt = df_snap.groupby([self.id_col, self.time_index_col]).agg(
            {'recency_weights': np.sum}).unstack(level=self.time_index_col).reset_index(drop=True)

        # self.temporal_known_num_col_list processing
        temporal_known_num_col_dict = {}
        for known_col in self.temporal_known_num_col_list:
            df_snap_known_num_col = df_snap.groupby([self.id_col, self.time_index_col]).agg(
                {known_col: np.sum}).unstack(level=self.time_index_col).reset_index(drop=True)
        ...
        for unknown_col in self.temporal_unknown_num_col_list:
            df_snap_unknown_num_col = df_snap.groupby([self.id_col, self.time_index_col]).agg(
                {unknown_col: np.sum}).unstack(level=self.time_index_col).reset_index(drop=True)
        ...
        for known_onehot_col in self.known_onehot_cols:
            df_snap_known_onehot_col = df_snap.groupby([self.id_col, self.time_index_col]).agg(
                {known_onehot_col: np.sum}).unstack(level=self.time_index_col).reset_index(drop=True)
        ...
        for unknown_onehot_col in self.unknown_onehot_cols:
            df_snap_unknown_onehot_col = df_snap.groupby([self.id_col, self.time_index_col]).agg(
                {unknown_onehot_col: np.sum}).unstack(level=self.time_index_col).reset_index(drop=True)
        ...
        logger.info("df_snap pre-dedup size: ", df_snap.shape)
        ...
        logger.info("df_snap post-dedup size: ", df_snap.shape)
        logger.info("df_snap columns: ", df_snap.columns)

        # index nodes
        col_map_dict = self.node_indexing(df_snap,
                                          [self.id_col] + self.static_cat_col_list + self.global_context_col_list)

        # map id to indices
        for col, id_map in col_map_dict.items():
            df_snap[col] = df_snap[col].map(id_map["index"]).astype(int)

        # get minimum set of target node indices for subgraph extraction & batched training
        pivot_node_index_list_of_lists = []
        for col in self.static_cat_col_list:
            col_unique_values = df_snap[col].unique().tolist()
            for value in col_unique_values:
                pivot_node_index_list = df_snap[df_snap[col] == value][self.id_col].unique().tolist()
                pivot_node_index_list_of_lists.append(pivot_node_index_list)

        # set cover solution using greedy method
        self.pivot_leaf_node_indices = greedy_set_cover(pivot_node_index_list_of_lists)

        # convert 'key_list' to key indices
        df_snap = df_snap.assign(mapped_key_list=[[col_map_dict[self.id_col]['index'][k] for k in literal_eval(row) if
                                                   col_map_dict[self.id_col]['index'].get(k)] for row in
                                                  df_snap['key_list']])
        ...
        data[self.target_col].y_weight = torch.tensor(df_snap['Key_Weight'].to_numpy().reshape(-1, 1),
                                                      dtype=torch.float)
        data[self.target_col].y_level_weight = torch.tensor(df_snap['Key_Level_Weight'].to_numpy().reshape(-1, 1),
                                                            dtype=torch.float)
        data[self.target_col].y_mask = torch.tensor(df_snap_mask.to_numpy(),
                                                    dtype=torch.float)  # shape (n_key, n_period)
        data[self.target_col].y_outlier_mask = torch.tensor(df_snap_outlier_mask.to_numpy(),
                                                            dtype=torch.float)  # shape (n_key, n_period)

        if len(self.scaler_cols) == 1:
            data['scaler'].x = torch.tensor(df_snap['scaler'].to_numpy().reshape(-1, 1), dtype=torch.float)
            # for batched_train only
            ...
            # for batched_train only
            data[self.target_col].scaler = torch.tensor(df_snap[self.scaler_cols].to_numpy().reshape(-1, 2),
                                                        dtype=torch.float)
        ...
        if self.recency_weights:
            data[self.target_col].recency_weight = torch.tensor(df_snap_recency_wt.to_numpy(),
                                                                dtype=torch.float)  # shape (n_key, n_period)

        # get keybom for index_select in the model
        data['keybom'].x = keybom_padded

        ################## artefacts for efficiency start
        # get list of key_levels where first element is covar_key_level
        ...
        kl_list = df_snap['key_level'].unique().tolist()
        sorted_kl_list = sorted(kl_list, key=sorter(self.covar_key_level))

        # generate key_level conditions list
        kl_conditions = [df_snap['key_level'] == i for i in sorted_kl_list]
        kl_choices = [int(i) for i in range(len(sorted_kl_list))]
        ...
        logger.info("generated key level indices with unique levels as: ", sorted_kl_list)
        ...
        data[self.target_col].key_level_index = torch.tensor(df_snap['key_level_index'].to_numpy().reshape(-1, 1),
                                                             dtype=torch.int16)
        data[self.target_col].key_level_count = torch.tensor(df_snap['key_level_count'].to_numpy().reshape(-1, 1),
                                                             dtype=torch.int16)
        ...
        df_global_var_snap = pd.concat([df_global_var_snap[self.global_context_col_list],
                                        pd.get_dummies(data=df_global_var_snap, columns=self.global_context_col_list,
        ...
        logger.info("keybom shape: ", data['keybom'].x.shape)
    ...
    def create_train_test_dataset(self, df):
        ...
        logger.info("new preprocessed temporal_unknown_num_col_list: ", self.temporal_unknown_num_col_list)
            ...
            if self.subgraph_sample_col is not None:
                ...
                if self.subgraph_sample_size > 0:
                    for i in range(0, len(all_subgraph_col_values), int(self.subgraph_sample_size)):
                        ...
                        logger.info("create subgraph for: ", all_subgraph_col_values[i:i + self.subgraph_sample_size])
    ...
    def create_infer_dataset(self, df, infer_start):
        ...
        infer_start_cutoff = \
        sorted(df[df[self.time_index_col] < infer_start][self.time_index_col].unique(), reverse=False)[
    ...
    def split_train_test(self, data):
        ...
        logger.info("train & test cutoffs: ", train_cutoff, test_start, test_cutoff)
    ...
    def get_sub_batch_sample(self, batch):
        ...
        sub_batch[self.target_col].y_level_weight = torch.clamp(
            sub_batch[self.target_col].y_level_weight * (sub_batch_num_target_nodes / self.num_target_nodes), min=1)
        ...
        for i, index in enumerate(iterator):
            ...
            for node_type in node_types_list:
                if node_type == self.target_col:
                    ...
                    batch[node_type].x = batch[node_type].x[:, index - context_len:index].unsqueeze(dim=2)
                    ...
                    batch[node_type].x = batch[node_type].x[:, index - self.max_covar_lags:index].unsqueeze(dim=2)
                elif node_type in ['scaler', 'keybom', 'key_aggregation_status', 'tweedie_p']:
                    pass
                elif node_type in self.global_context_col_list:
                    batch[node_type].x = batch[node_type].x.unsqueeze(dim=1).repeat(1, context_len + self.fh, 1)
                else:
                    batch[node_type].x = batch[node_type].x[:,
                                         index - self.max_covar_lags:index + self.max_leads].unsqueeze(dim=2)
    ...
    def build(self,
        ...
        logger.info("graph metadata: ", self.metadata)

        if batched_train:
            indices_list = self.pivot_leaf_node_indices[0]
            indices_tensor = torch.tensor(indices_list, dtype=torch.long)
            loader = NeighborLoader(sample_batch,
                                    num_neighbors={key: [-1] if key[0] != key[2] else [0] for key in
                                                   sample_batch.edge_types},
                                    batch_size=len(indices_list),
                                    input_nodes=(self.target_col, indices_tensor),
        ...
        try:
            ...
            logger.info("total model params: ", pytorch_total_params)
        except:
    ...
    def train(self,
        ...
        clip_gradients = scheduler_params['clip_gradients']
        max_norm = scheduler_params['max_norm']
        norm_type = scheduler_params['norm_type']
        ...
        def train_fn():
            ...
            for i, batch in enumerate(batch_gen):
                ...
                if stop_training_criteria == 'mse':
                    ...
                    total_metric += float(mse_err)
                elif stop_training_criteria == 'mae':
                    mae_err = (mask * torch.abs(out - actual)).mean().data
                    total_metric += float(mae_err)
                elif stop_training_criteria == 'loss':
                    total_metric += float(weighted_loss)
        ...
        for epoch in range(max_epochs):
            ...
            if stop_training_criteria == 'loss':
                ...
                # compare metric
                if epoch == 0:
                    prev_min_metric = np.min(val_metric_hist)
                else:
                    prev_min_metric = np.min(val_metric_hist[:-1])
    ...
    def batched_train(self,
        ...
        num_batches = len(self.pivot_leaf_node_indices)

        print("\nUsing batched training to save gpu memory.\n")
        print(f"num batches per graph: {num_batches} \n")
        # print(f"pivot node indices: {self.pivot_leaf_node_indices} \n")

        if max_batch_size is not None:
            # merge pivot_leaf_node_indices into fewer subsets
            pivot_leaf_node_indices_subsets = list_of_lists_to_set_of_subsets(self.pivot_leaf_node_indices)
            pivot_leaf_node_indices_merged_sets = merge_subsets_min_bins(pivot_leaf_node_indices_subsets,
                                                                         max_batch_size)
            pivot_leaf_node_indices_merged_lists = list_of_sets_to_list_of_lists(pivot_leaf_node_indices_merged_sets)

            print(f"new num batches per graph: {len(pivot_leaf_node_indices_merged_lists)} \n")
        else:
            pivot_leaf_node_indices_merged_lists = self.pivot_leaf_node_indices
        ...
        def train_fn():
            ...
            batch_gen = self.batch_generator(self.train_dataset, 'train', self.device)
            print("train loop...")
            for batch_num, full_batch in enumerate(batch_gen):
                print(f"batch no. {batch_num}")
                for i, indices_list in enumerate(pivot_leaf_node_indices_merged_lists):
                    indices_tensor = torch.tensor(indices_list, dtype=torch.long)
                    loader = NeighborLoader(full_batch,
                                            num_neighbors={key: [-1] if key[0] != key[2] else [0] for key in
                                                           full_batch.edge_types},
                                            batch_size=len(indices_list),
                                            input_nodes=(self.target_col, indices_tensor),
                                            subgraph_type="induced",
                                            replace=False)
                    ...
                    actual = torch.unsqueeze(batch[self.target_col].y, dim=2) * torch.unsqueeze(
                        batch[self.target_col].scaler, dim=2)
