...
from ..models.stgnn_sequence import STGNN
...
class graphmodel:
    ...
    def onehot_encode(self, df):
        ...
        df = pd.concat([df[onehot_col_list], pd.get_dummies(data=df, columns=onehot_col_list, prefix_sep='_')], axis=1,
                       join='inner')
    ...
    def create_snapshot_graph(self, df_snap, period):
        ...
        for col in self.global_context_col_list:
            ...
            for value in col_unique_values:
                ...
                edges = df_snap[(df_snap[col] == value) & (df_snap['key_level'] == self.covar_key_level)][
                    [col, self.id_col]].to_numpy()
    ...
    def batch_generator(self, graph_dataset, mode, device):
        ...
        for i, index in enumerate(iterator):
            ...
            for node_type in node_types_list:
                ...
                elif node_type in ['scaler', 'keybom', 'key_aggregation_status',
                                   'tweedie_p'] + self.global_context_col_list:
                    pass
                else:
    ...
    def build(self,
        ...
        # build model
        self.base_model = STGNN(feature_dim=feature_dim,
                           ...
                           chunk_size=chunk_size).to(self.device)
        ...
        if self.gbt:
            if self.n_boosters >= 1:
                ...
                for n in range(self.n_boosters):
                    self.boost_model_dict[f"boost_model_{n}"] = STGNN(feature_dim=feature_dim,
                                                                   hidden_channels=model_dim,
                                                                   metadata=self.metadata,
                                                                   target_node=self.target_col,
                                                                   feature_extraction_node_types=self.feature_extraction_col_list,
    ...
    def train_workflow(self,
              ...
              min_delta,
              model_prefix,
              loss_type='Quantile',  # 'Tweedie','Huber','RMSE'
              delta=1.0,  # for Huber
              use_amp=False,
        ...
        if self.loss == 'Tweedie':
            ...
            if len(self.tweedie_variance_power) >= 1:
                self.tweedie_variance_power = [torch.tensor(i, dtype=torch.float32).reshape([1, 1]).to(self.device) for
                                               i in self.tweedie_variance_power]
    ...
    def infer(self, infer_start, infer_end, select_quantile):
        ...
        if not self.gbt:
            ...
            logger.info("running inference using best saved base model: {} & boost models: {}".format(self.best_base_model,
                                                                                              self.best_boost_model_dict))
